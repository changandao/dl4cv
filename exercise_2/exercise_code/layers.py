...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    M = b.shape[0]
    x_tempt = x.reshape(N, int(x.size/N))
    out = np.dot(x_tempt,w) + b#+ np.dot(np.ones((N,1)),b.reshape(1,M)) 
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b)
...
def affine_backward(dout, cache):
    ...
    x_tempt = x.reshape(x.shape[0], int(x.size/x.shape[0]))
    dw = np.dot(x_tempt.T, dout)
    db = np.sum(dout, 0)#/x.shape[0]
...
def batchnorm_backward(dout, cache):
    ...
    divar = np.sum(dx_norm * x_minus_mean,0)
    dsqtvar = divar * 1./ (-sqrtvar*sqrtvar)
    dvar = dsqtvar * 0.5 / sqrtvar
    dsq =  np.ones((N,D))*dvar/N
    dx_minus_mean2 = dsq *2*x_minus_mean
    dx_minus_mean = dx_minus_mean1 + dx_minus_mean2
    dx2 = -np.sum(dx_minus_mean,0)
    dx = dx_minus_mean + dx2*np.ones((N,D))/N
