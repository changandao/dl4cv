...
class Solver(object):
    ...
    def train(self, model, train_loader, val_loader, num_epochs=10, log_nth=0):
        ...
        for epoch in range(num_epochs):
            ...
            for i, (input, target) in enumerate(train_loader, 0):
                ...
                optim.zero_grad()
                output = model(input)
                loss = self.CrossEntropy2d(output, target)
                loss.backward()
                optim.step()
                ...
                if t % log_nth == 0:
                    print('(Iteration %d / %d) loss: %f' % (
                        t , num_iterations, self.train_loss_history[-1]))
                if i == iter_per_epoch-1:
                    _, pred = torch.max(output, 1)
                    
                    target_mask = target>0
                    train_acc = np.mean((pred == target)[target_mask].data.cpu().numpy())
            ...
            for input, target in val_loader:
                ...
                target_mask = target>0
                val_acc = np.mean((pred == target)[target_mask].data.cpu().numpy())
    ...
    def CrossEntropy2d(self, input, target, weight=None, size_average=True):
        ...
        n, c, h, w = inputs.size()
        # This will just make the channel as the last dimension
        inputs = inputs.transpose(1, 2).transpose(2,3).contiguous()
        #inputs = inputs.transpose(2, 3).contiguous()
        #repeat method repeats tensor number of times along the given axis
        #We are doing this so that target becomes the same dim as the input
        # >=0 check is for removing unlabeled pixels
